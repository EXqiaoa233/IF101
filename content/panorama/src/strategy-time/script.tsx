...
export const Strategy = () => {
    useEffect(() => {
        ...
        if (DotaHUD.FindChildTraverse('SelectedHeroAbilitiesHitTargets')) DotaHUD.FindChildTraverse('SelectedHeroAbilitiesHitTargets')!.visible = false
    ...
    useEffect(() => {
        ...
        shl(Object.entries(Object.entries(strategy_data.player).find(v => v[1].pid == Players.GetLocalPlayer())![1].abihero).map(v => { return { abi: Object.entries(v[1].abi).map(v => v[1]), hero: v[1].hero } }))
    }, [strategy_data])
    ...
    return useMemo(() => (<>
        <Panel className="top" hittest={false}>
            <Panel style={{ backgroundImage: "url('file://{images}/strategy/tbg.png')", width: '60px', height: '60px', backgroundSize: 'contain', backgroundRepeat: 'no-repeat', marginLeft: GetFitWidth(1780), marginTop: '23px' }}>
        ...
        <Panel style={{ marginLeft: GetFitWidth(770), marginTop: GetFitHeight(172), width: GetFitWidth(559), height: GetFitHeight(496), backgroundColor: ' rgba(0, 0, 0, 0.40)', flowChildren: 'down' }}>
            {
                hl.map(v => (
                    <Panel style={{ marginTop: '16px', marginLeft: '16px', width: '527px', height: '80px' }}>
                        ...
                        {
                            v.abi.map((v, i) => <DOTAAbilityImage className="playerabiHover" style={{ marginLeft: `${142 + 16 + 96 * i}px`, width: '80px', height: '80px' }} abilityname={v} showtooltip={true} onactivate={(e) => {
                                if (e.abilityname != 'rubick_empty1') {
                                    sabi(e.abilityname)
                                }
                            }} >
                                {
                                    (i != 3 ? (pd.includes(v) || pd.find((v, i) => v == 'rubick_empty1' && i != 3) == undefined) : pd.indexOf(v) != -1) && <Panel style={{ width: '100%', height: '100%', backgroundColor: 'rgba(0, 0, 0, 0.9)' }} className="easingAai">
                                        {
                                            pd.indexOf(v) != -1 && <Label text={pd.indexOf(v) + 1} style={{ fontSize: '45px', align: 'center center', color: 'white' }} className="easingText" />
                                        }
        ...
        <Panel style={{ width: GetFitWidth(402), height: GetFitHeight(899), marginLeft: GetFitWidth(1518), marginTop: GetFitHeight(126), backgroundColor: ' rgba(0, 0, 0, 0.40)' }}>
            ...
            <Panel style={{ marginTop: '30px', width: '100%', flowChildren: 'down', overflow: 'squish scroll' }}>
                {
                    tm.map(v => (
                        <Panel style={{ width: '372px', height: '163px', marginTop: '15px', marginLeft: '20px', backgroundImage: "url('file://{images}/strategy/pbg.png')", backgroundSize: 'cover', backgroundRepeat: 'no-repeat' }}>
                            <DOTAAvatarImage steamid={v.sid == '0' ? '76561198364387387' : v.sid} style={{ width: '60px', height: '60px', marginLeft: '15px', marginTop: '13px' }} />
                            <DOTAHeroImage style={{ width: '106px', height: '60px', marginLeft: '90px', marginTop: '13px' }} heroname={v.hero} heroimagestyle='landscape' />
                            <Label text={v.name} style={{ marginLeft: '212px', marginTop: '20px' }} className="teammateName" />
                            <Label text={$.Localize(`#${v.hero}`)} style={{ marginLeft: '212px', marginTop: '46px' }} className="teammateHeroName" />
                            {
                                v.abi.map((v, i) => <DOTAAbilityImage style={{ width: '60px', height: '60px', marginLeft: `${15 + 75 * i}px`, marginTop: '88px' }} abilityname={v} showtooltip={true} />)
                            }
                        </Panel>
                    ))
                }
        ...
        {
            pd.map((v, i) => <DOTAAbilityImage className="playerabiHover" style={{ width: '64px', height: '64px', marginLeft: GetFitWidth(190 + i * 76), marginTop: GetFitHeight(222) }} showtooltip={true} abilityname={v} onactivate={(e) => {
                if (e.abilityname != 'rubick_empty1') {
                    sabi(e.abilityname)
                }
            }} draggable={true}
        ...
        <Button className="refreshButton" style={{ width: GetFitWidth(200), height: GetFitHeight(60), marginLeft: GetFitWidth(950), marginTop: GetFitHeight(688), backgroundColor: 'gradient( linear, 0% 0%, 0% 100%, from( #DAC480 ), to( #5B3E0D ) )', borderRadius: '5px' }} onactivate={() => {
