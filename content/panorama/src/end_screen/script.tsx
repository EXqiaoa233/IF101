...
const Item = (props: { title: string; items: string[] }) => {
    ...
    if (title == 'abi') {
        return (
            <Panel style={{ marginLeft: '12px', height: '100%' }}>
                ...
                <Panel style={{ marginTop: '70px', flowChildren: 'down' }}>
                    {items.map(v => (
                        <Panel style={{ height: '66px', marginTop: '4px', flowChildren: 'right' }}>
    ...
    if (title == 'kill') {
        return (
            <Panel style={{ marginLeft: '12px', height: '100%' }}>
                ...
                <Panel style={{ marginTop: '70px', flowChildren: 'down' }}>
                    {items.map(v => (
                        <Panel style={{ height: '66px', marginTop: '4px' }}>
            ...
            <Panel style={{ marginLeft: '12px', height: '100%' }}>
                <Panel style={{ height: '70px' }}>
                    <Label
                        text={$.Localize(`#${title}`)}
                        html={true}
                        style={{ align: 'center center', color: '#E7BA01FF', fontWeight: 'bold' }}
                        className="titleText"
                    />
                </Panel>
                <Panel style={{ marginTop: '70px', flowChildren: 'down' }}>
                ...
                <Panel style={{ width: GetFitWidth(1920), height: GetFitHeight(770), backgroundColor: '#000000B7', marginTop: GetFitHeight(180) }}>
                    <Panel
                        style={{
                            ...
                            marginLeft: '32px',
                            height: '100%',
                            backgroundColor: 'gradient( linear, 0% 0%, 0% 100%, from( #222424 ), to( #121A1F ) )',
                        }}
                    >
                        <Label
                            text={`点击切换${team == 0 ? `夜魇` : `天辉`}`}
                            style={{ fontSize: '20px', marginLeft: '180px', marginTop: '23px', textShadow: '0px 0px 4px 1.0 #BDB76B' }}
                            className="exText"
                        />
                        <Panel
                        ...
                        >
                            <Panel
                                className={`${team == 0 ? `dire` : `radiant`}Img`}
                                style={{ opacity: '0.1', marginTop: '10px', marginLeft: '6px' }}
                            />
                            <Panel className={`${team == 0 ? `radiant` : `dire`}Img`} />
                            <Label
                                text={team == 0 ? `天辉` : `夜魇`}
                                className="teamText"
                                style={{ textShadow: team == 0 ? '0px 0px 6px 1.0 #9BE40C8F' : '0px 0px 6px 1.0 #E74D088F' }}
                            />
                            <Label
                                text={`人头数： <font color="white"><b>${12}</b></font>`}
                                style={{ fontSize: '18px', marginLeft: '60px', marginTop: '30px' }}
                                html={true}
                            />
                        </Panel>
                    </Panel>
                    ...
                    <Panel style={{ marginTop: '70px', height: '700px', marginLeft: '32px', width: '100%', flowChildren: 'down' }}>
                        {new Array(10).fill(1).map(v => (
                            <Panel style={{ width: '100%', height: '70px' }}>
                                <Panel style={{ width: '318px', height: '66px', marginTop: '4px', backgroundColor: '#00000077' }}>
                                    ...
                                    <Label
                                        text={$.Localize(`#${'npc_dota_hero_axe'}`)}
                                        style={{ marginLeft: '88px', marginTop: '32px', fontSize: '16px' }}
                                    />
                                </Panel>
                                <Panel
                                    style={{
                                ...
                                ></Panel>
                                <Panel
                                    style={{ width: '260px', marginLeft: '1588px', height: '66px', marginTop: '4px', backgroundColor: '#00000077' }}
                                ></Panel>
                            </Panel>
                        ))}
                    </Panel>
                    ...
                    <Panel style={{ width: '1270px', marginLeft: '350px', height: '100%', flowChildren: 'right', overflow: 'scroll squish' }}>
                        {titles.map(v => (
                            <Item title={v} items={new Array(10).fill('1000000')} />
                        ))}
                    </Panel>
                </Panel>
            </>
